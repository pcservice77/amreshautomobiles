...
import { Search, Eye, Calendar, UserCheck, FileText, Printer, X, Zap } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <Dialog open={!!selectedSale} onOpenChange={(open) => !open && setSelectedSale(null)}>
        <DialogContent className={cn(
          ...
          <div 
            className="print-container relative bg-white" 
          ...
          >
            ...
            {/* Content Overlay */}
            <div className="relative z-10 w-full h-full p-[12mm]">
              {isTemplateMode ? (
                <div className="relative w-full h-full text-[10px] font-bold text-black uppercase">
                  {/* Absolute Positioning for the specific template provided by user */}
                  
                  {/* Invoice Meta */}
                  <div className="absolute top-[28mm] right-[10mm] text-red-600 font-black text-[14px]">
                    {selectedSale?.invoiceNo}
                  </div>
                  <div className="absolute top-[35mm] right-[10mm]">
                    {selectedSale?.soldAt ? format(new Date(selectedSale.soldAt), 'dd/MM/yyyy') : ''}
                  </div>

                  {/* Customer Details Section (Mapped to User Boxes) */}
                  <div className="absolute top-[52mm] left-[5mm] w-[85mm] space-y-1 leading-tight">
                    <p className="text-[12px] font-black">{selectedSale?.customerName}</p>
                    <p className="text-[9px]">{selectedSale?.customerFatherName || '-'}</p>
                    <p className="h-[10mm] overflow-hidden text-[9px]">{selectedSale?.address}, {selectedSale?.city}, {selectedSale?.state} - {selectedSale?.pin}</p>
                    <p className="text-[11px]">{selectedSale?.mobile}</p>
                    <p className="text-[9px] lowercase">{selectedSale?.email || '-'}</p>
                    <p className="text-[10px] mt-1">{selectedSale?.idNumber || '-'}</p>
                  </div>

                  {/* Vehicle Details Section (Mapped to User Boxes) */}
                  <div className="absolute top-[52mm] left-[105mm] w-[90mm] grid grid-cols-2 gap-y-1 gap-x-2 text-[9px]">
                    <div className="col-span-2 text-[12px] font-black text-primary">{selectedSale?.model}</div>
                  ...
                  {/* Table Area (Starts around 110mm) */}
                  <div className="absolute top-[108mm] left-0 w-full px-2">
                     <div className="grid grid-cols-[35px_1fr_100px_60px_100px_100px] gap-2 items-center py-2">
                        <div className="text-center">1</div>
                        <div className="font-black">{selectedSale?.model} {selectedSale?.variant}</div>
                  ...
                  {/* Pricing Summary (Bottom Right Section) */}
                  <div className="absolute bottom-[85mm] right-[10mm] w-[50mm] space-y-2 text-right text-[12px] font-black">
                     <p>₹ {selectedSale?.price.toLocaleString()}.00</p>
                     <p>₹ 0.00</p>
                     <p>₹ {selectedSale?.price.toLocaleString()}.00</p>
                     <div className="pt-8 text-[16px] text-primary">
                        ₹ {selectedSale?.price.toLocaleString()}.00
                     </div>
                  </div>

                  {/* Total In Words (Bottom Left) */}
                  <div className="absolute bottom-[85mm] left-[10mm] w-[120mm]">
                    <p className="text-[10px] italic">Amount in Words:</p>
                    <p className="text-[12px] font-black leading-tight mt-1">
                      {amountToWords(selectedSale?.price)} Only
                    </p>
                  </div>

                  {/* Payment Info */}
                  <div className="absolute bottom-[40mm] left-[10mm] text-[9px] space-y-1">
                    <p>Payment Method: {selectedSale?.paymentMethod}</p>
                    {selectedSale?.paymentMethod === 'Finance' && (
                      <p>Financier: {selectedSale?.financeCompany} | Loan: ₹ {selectedSale?.loanAmount}</p>
                    )}
                ...
                <>
                  {/* Standard Professional Invoice Header */}
                  ...
                  <div className="mb-6">
                    <table className="w-full invoice-table border-collapse">
                      <thead>
                        <tr>
                          <th className="w-12 text-center border-black">Sr</th>
                          <th className="text-left border-black">Item Description</th>
                          <th className="w-24 text-center border-black">HSN</th>
                          <th className="w-16 text-center border-black">Qty</th>
                          <th className="w-28 text-right border-black">Rate</th>
                          <th className="w-32 text-right border-black">Amount</th>
                        </tr>
                      </thead>
                      <tbody>
                        <tr className="h-40 align-top">
                          <td className="text-center font-bold border-black">1</td>
                          ...
                          <td className="text-center border-black">{selectedSale?.hsn || '871160'}</td>
                          <td className="text-center border-black">1</td>
                          <td className="text-right border-black">₹ {selectedSale?.price.toLocaleString()}.00</td>
                          <td className="text-right font-bold border-black">₹ {selectedSale?.price.toLocaleString()}.00</td>
                        </tr>
                        <tr className="summary-total text-white">
                          <td colSpan={4} className="py-4 px-6 text-right text-xl font-black italic border-none bg-primary">GRAND TOTAL</td>
                          <td colSpan={2} className="py-4 px-6 text-right text-2xl font-black border-none bg-primary">₹ {selectedSale?.price.toLocaleString()}.00</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
                  ...
                  {/* Bottom Info */}
                  <div className="grid grid-cols-2 gap-8 mb-8 text-[11px] text-black">
                    <div>
                  ...
                  {/* Signatures */}
                  <div className="flex justify-between items-end mt-20 px-4 text-black">
