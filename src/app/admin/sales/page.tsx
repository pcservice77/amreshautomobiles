  ...
  return (
    <div className="space-y-6">
      ...
      <Dialog open={!!selectedSale} onOpenChange={(open) => !open && setSelectedSale(null)}>
        <DialogContent className="max-w-[210mm] max-h-[95vh] overflow-y-auto bg-white text-black p-0 border-none print:shadow-none print:m-0">
          ...
          <div className="print-container p-[12mm] relative" id="printable-invoice">
            <div className="invoice-watermark">AMRESH</div>
            
            {/* Header */}
            {!showroom?.useLetterhead && (
              <div className="flex justify-between border-b-2 border-primary pb-4 mb-6">
                ...
                <div className="text-right flex flex-col justify-between">
                  ...
                  <div className="flex gap-2 justify-end opacity-20">
                     <Zap className="h-4 w-4" /><Zap className="h-4 w-4" /><Zap className="h-4 w-4" />
                  </div>
                </div>
              </div>
            )}

            {showroom?.useLetterhead && <div className="h-[45mm]" />}
            ...
            <div className="mb-6">
              <table className="w-full invoice-table border-collapse">
                ...
                <tbody>
                  ...
                  <tr>
                    <td colSpan={4} className="border-none text-right pt-4 px-4 font-bold text-gray-600">SUBTOTAL</td>
                    <td colSpan={2} className="border-none text-right pt-4 px-4 font-bold">₹ {selectedSale?.price.toLocaleString()}.00</td>
                  </tr>
                  <tr>
                    <td colSpan={4} className="border-none text-right px-4 font-bold text-gray-600">TAX (Included)</td>
                    <td colSpan={2} className="border-none text-right px-4 font-bold">₹ 0.00</td>
                  </tr>
            ...
            <div className="grid grid-cols-2 gap-8 mb-8 text-[11px]">
              <div>
                ...
                <div className="mt-6 border-t-2 border-gray-100 pt-3">
                  <h5 className="font-black text-primary uppercase text-[9px] mb-2 tracking-widest">Payment Information</h5>
                  <div className="grid grid-cols-2 gap-2 text-[10px]">
                    <p><span className="text-gray-500 font-semibold">Method:</span> {selectedSale?.paymentMethod}</p>
                    {selectedSale?.utrNumber && <p><span className="text-gray-500 font-semibold">Ref:</span> {selectedSale.utrNumber}</p>}
                    {selectedSale?.paymentMethod === 'Finance' && (
                      <>
                        <p><span className="text-gray-500 font-semibold">Financer:</span> {selectedSale.financeCompany}</p>
                        <p><span className="text-gray-500 font-semibold">Loan Amt:</span> ₹ {selectedSale.loanAmount?.toLocaleString()}</p>
                      </>
                    )}
                  </div>
                </div>
            ...
            <div className="mt-20 pt-6 border-t border-gray-100 text-center">
              <p className="text-primary font-black text-xs uppercase tracking-[0.4em]">Thank you for choosing Amresh Automobile</p>
              <p className="text-[9px] text-gray-400 font-bold mt-1 uppercase">Green Energy • Smart Future • Drive Electric</p>
            </div>
          </div>
        </DialogContent>
      </Dialog>
